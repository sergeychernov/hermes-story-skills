...
3. **Verify the copy.** Compare source and archive SHA-256. For images, record byte size and dimensions using `scripts/archive_photo.py`. For videos, use `ffprobe` to record duration, dimensions, codecs, frame rate, audio streams, creation time and size; generate a separate contact sheet only for review.
   **Protect spoken-video integrity.** Treat speech as part of the source contract, not as disposable lead-in or tail. Keep a conversational clip's full timeline unless the user explicitly approves a trim after hearing or reviewing it; visual darkness or apparent inactivity is not evidence that the audio is expendable. If the user says “leave the whole clip,” remove every start/end trim from the derivative and compare source/output video and audio stream start times and durations. Apply requested denoise and loudness normalization only to derivatives, never originals; avoid gates or silence removal that can cut quiet syllables. Verify integrated loudness, true peak/no clipping, A/V alignment, and that output audio spans the full source speech before presenting the result.
...
7. **Treat short media messages as continuation—unless the user declares a boundary.** During an active archive sequence, text such as “кактусы на улице”, “пасутся гуси” or “видео приветствие” normally supplies the title/context and means append it. **Attachment binding outranks processing order:** when one message contains both text and a media attachment, bind that text to that attached media even if the agent is still processing the preceding upload. Do not attach it to the previous material merely because that turn remains unresolved. Reconcile any mid-turn correction against the actual message/attachment boundary before patching manifests; undo a speculative reassignment consistently across journal, title choices, and media manifest. Do not repeatedly ask where to save or whether to continue. However, phrases such as **“новая сессия”**, **“новый архив”**, **“новая публикация”**, or clear equivalents are hard reset markers: start a separate archive/package with the media attached to that boundary message (or the immediately following media), and do **not** backfill preceding chat images, videos, captions, inferred locations, or story context unless the user explicitly asks to import them. A new publishing target (for example, “новая публикация Shorts”) sets the target and collection state; it does not itself authorize publication.
...
10. **Prepare cross-platform exports.** When the user asks to assemble the story, create an Instagram package and a vertical Reel/YouTube Shorts package from the same source set. Follow `references/video-and-publishing-pipeline.md` for render layout, metadata manifest, audio rights and verification.
   **Reserve a face-free title zone in dense collages.** Decide the panel layout from subject geometry before rendering, not from equal-cell convenience. Keep portraits and key actions above the lower fifth, and place a face-free landscape/detail panel (for example water, reflections, pavement, or foliage) across the lower title zone. For five-image vertical collages, a dense `2 + 2 + 1` layout is often safer than `2 + 1 + 2`: the full-width bottom panel can carry the title while all people remain unobstructed. Require edge-to-edge coverage with no blurred filler or empty cells, complete entrances within the requested animation window, then inspect an in-motion frame plus the completed and final frames.
