...
Choose cover duration by *target surface*, not merely by a request to “hit the preview”. A one-frame cover is not allowed for Sergey's YouTube Shorts publication masters because Studio may not expose it as a selectable frame after transcoding. The default and required publication contract is **exactly 4 cover frames at 30 fps** unless Sergey explicitly approves a longer intro. Render frames `0`, `1`, `2`, and `3` from the same approved safe-zone-compliant cover image with no black/gray lead-in; frame `4` must be the first live frame. Lossy H.264 frames need not have byte-identical decoded PNG hashes, so verify visual equivalence with an exact `0..4` contact sheet plus high SSIM rather than demanding byte identity. A longer intro of roughly 0.5–0.8 seconds is a separate explicit override, never an automatic fallback.
