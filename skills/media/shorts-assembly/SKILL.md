...
Before assembling a mixed-media story, classify every scene as `voice`, `music`, or `silent`. Render generated accompaniment as separate `rhythm` and `melody` stems whenever routing differs by scene class. For Sergey's family-film workflow, the default matrix is: `voice` = cleaned original audio + quiet rhythm, melody off; `music` = conservatively cleaned and normalized source audio only, both generated stems off; `silent` = rhythm + melody. Do not rely on ducking to hide competing melodies. Denoise and normalize each real video individually, and verify actual post-encode LUFS and true peak. If rhythm or a named style such as swing is requested, encode its actual mechanics—BPM, pulse, accent pattern, and timing ratio—not melody alone.
...
**Audio-only approval files must be timeline-exact.** AAC encoder padding may make a nominal 59.9-second mix probe as 60.014 seconds. Build and assert a sample-exact PCM master first (`expected_seconds × sample_rate` frames), trim with `atrim=end_sample=...`, then encode the listening AAC with an explicit target duration and re-probe it. When `loudnorm` precedes sample-count trimming, remember that FFmpeg may negotiate 192 kHz internally: insert an explicit `aresample=48000` **before** `atrim=end_sample=...`, otherwise the same end-sample value can silently produce one quarter of the intended duration. Do not approve or mux an overlong or underlong audio file on the assumption that the video container will hide or restore the tail.
...
If the intro needs a new musical sting, render and deliver the **audio-only ident first** for approval before spending time on video assembly. Make it original and deterministic; do not delegate its composition to a coding agent.
