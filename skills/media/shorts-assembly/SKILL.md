...
**Safe zones.** Use the machine-readable platform contract emitted by `static-cover-collage`. For `youtube_shorts_cover`, reject the artifact unless every critical text bounding box is inside its central YouTube-Shorts/Telegram-OG crop-safe rectangle. Numeric UI-safe margins are locally calibrated or conservative policies unless the cited platform source explicitly publishes them; never present local percentages as official specifications.

Choose cover duration by *target surface*, not merely by a request to “hit the preview”. A YouTube Shorts publication master uses **exactly 4 cover frames at 30 fps**. Render frames `0`, `1`, `2`, and `3` from the same approved safe-zone-compliant cover image with no black/gray lead-in; frame `4` must be the first live frame. One-frame and longer cover intervals are invalid for this publication path. A separately requested visible intro is a different non-publication edit and requires a distinct timeline/soundtrack approval before it can replace this contract. Lossy H.264 frames need not have byte-identical decoded PNG hashes, so verify visual equivalence with an exact `0..4` contact sheet plus high SSIM rather than demanding byte identity.
...
**Local cover swap with locked audio:** for a YouTube Shorts publication master, first prove that the encoded master already has the required four-frame interval. Replace only frames `0..3`, preserve total frame count, stream-copy the existing audio, and verify the exact `3 → 4` boundary. If the existing interval is not four frames, a cover-only locked-audio swap cannot produce a publication candidate; rebuild through a newly approved timeline/soundtrack contract instead of falsifying the report. Follow `references/cover-swap-with-locked-audio.md`.
...
**Publication-package handoff must follow the actual publisher parser, not a locally invented schema.** The live publisher requires a trusted schema, an approved hash-bound manifest, exact snapshot hashes, an eligible video report (`artifact`, accepted `status`, `video.full_decode=passed`, exact SHA-256, and `timeline.cover_frames=4` / `first_live_frame=4`), and a separately approved cover report. Immediately before OAuth it also decodes media frames `0..4`; report fields alone cannot satisfy the gate. Write one complete YouTube tag per non-empty line: a comma-separated list on one line is parsed as one tag. A local preflight failure means no upload began. After an upload-attempt journal exists, never retry until API readback resolves that attempt.
