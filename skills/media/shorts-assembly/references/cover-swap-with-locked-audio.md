...
For Sergey's YouTube Shorts publication masters, reject a one-frame cover. Use exactly four frames rendered from the same approved, central-crop-safe cover image at 30 fps unless Sergey explicitly requests a longer visible intro. Frames `0..3` are the cover and frame `4` is live footage. Verify visual equivalence after lossy encode rather than requiring byte-identical decoded frames.
