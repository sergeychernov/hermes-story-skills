...
import argparse, asyncio, getpass, json, os, tempfile
...
async def main() -> None:
    if TelegramClient is None:
        raise SystemExit('Telethon is required for Telegram authorization; install the social-publisher runtime dependencies')
    BASE.mkdir(parents=True,exist_ok=True); BASE.chmod(0o700)
...
def cli() -> None:
    argparse.ArgumentParser(description='Authorize a Telegram user session for Story publishing').parse_args()
    asyncio.run(main())
